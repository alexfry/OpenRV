...
// Only connect the update slot to the architecture appropriate signal
    ...
    void RvPreferences::update()
    {
        ...
        if (opts.stereoMode)
        {
            ...
            if (s == StereoAnaglyph)
                n = 1;
            else if (s == StereoPair)
                n = 2;
            else if (s == StereoMirror)
                n = 3;
            else if (s == StereoLeft)
                n = 4;
            else if (s == StereoRight)
                n = 5;
            else if (s == StereoChecker)
                n = 6;
        ...
        {
            // "area" is n=0.
            if (!strcmp(opts.resampleMethod, "cubic"))
                n = 1;
            else if (!strcmp(opts.resampleMethod, "linear"))
                n = 2;
            else if (!strcmp(opts.resampleMethod, "nearest"))
                n = 3;
        }
        m_ui.resampleMethodCombo->setCurrentIndex(n);

        if (opts.imageFilter == GL_LINEAR)
            m_ui.imageFilterCombo->setCurrentIndex(0);
        if (opts.imageFilter == GL_NEAREST)
            m_ui.imageFilterCombo->setCurrentIndex(1);

        n = 0;
        if (opts.bgpattern)
        {
            if (!strcmp(opts.bgpattern, "grey18"))
                n = 1;
            else if (!strcmp(opts.bgpattern, "grey50"))
                n = 2;
            else if (!strcmp(opts.bgpattern, "white"))
                n = 3;
            else if (!strcmp(opts.bgpattern, "checker"))
                n = 4;
            else if (!strcmp(opts.bgpattern, "crosshatch"))
                n = 5;
        }
        ...
        s.setNum(opts.acachesize);
        m_ui.audioCachePacketEdit->setText(s);
        s.setNum(opts.audioMinCache, 'g', 2);
        m_ui.audioCacheMinEdit->setText(s);
        s.setNum(opts.audioMaxCache, 'g', 2);
        m_ui.audioCacheMaxEdit->setText(s);
        s.setNum(opts.audioGlobalOffset, 'g', 4);
        m_ui.audioGlobalOffsetEdit->setText(s);
        s.setNum(opts.audioDeviceLatency, 'g', 4);
        m_ui.audioDeviceLatencyEdit->setText(s);
        // s.setNum(opts.audioRate, 'g', 10);
        // m_ui.audioDeviceRateEdit->setText(s);
        ...
        default:
        case 32:
            currentFormat = TwkAudio::Float32Format;
            break;
        case 24:
            currentFormat = TwkAudio::Int24Format;
            break;
        case 16:
            currentFormat = TwkAudio::Int16Format;
            break;
        case 8:
            currentFormat = TwkAudio::Int8Format;
            ;
            break;
        case -32:
            currentFormat = TwkAudio::Int32Format;
            break;
        }
    ...
    void RvPreferences::resetPreferencesFile()
    {
#ifdef PLATFORM_WINDOWS
    ...
    void RvPreferences::loadSettingsIntoOptions(Options& opts)
    {
        RV_QSETTINGS;
    ...
    void RvPreferences::loadSettingsIntoOptions(RvSettings& settings, Options& opts)
    {
        ...
        if (s == StereoAnaglyph)
            opts.stereoMode = (char*)StereoAnaglyph;
        if (s == StereoLumAnaglyph)
            opts.stereoMode = (char*)StereoLumAnaglyph;
        if (s == StereoPair)
            opts.stereoMode = (char*)StereoPair;
        if (s == StereoMirror)
            opts.stereoMode = (char*)StereoMirror;
        if (s == StereoChecker)
        ...
        if (s == StereoRight)
            opts.stereoMode = (char*)StereoRight;
        ...
        // Look for architecture specific preferences first, then fall back to
        // non-specific
        double tmplram = -1;
        double tmpcram = -1;
        ...
        //  threaded uploads are not allowed, later code will ensure the
        //  checkbox is disabled, etc.  But opts struct should represent state
        //  in settings file.
        //
        ...
        if (s == Area)
            ...
            opts.resampleMethod = (char*)Linear;
        else if (s == Cubic)
            opts.resampleMethod = (char*)Cubic;
        else if (s == Nearest)
            opts.resampleMethod = (char*)Nearest;

        s = settings.value("imageFilter", opts.imageFilter).toString();

        if (s == Linear)
            opts.imageFilter = GL_LINEAR;
        else if (s == Nearest)
        ...
        if (s == "black")
            ...
            opts.bgpattern = (char*)Solid100;
        else if (s == "grey18")
            opts.bgpattern = (char*)Solid18;
        else if (s == "grey50")
            opts.bgpattern = (char*)Solid50;
        else if (s == "checker")
            opts.bgpattern = (char*)Checker;
        else if (s == "crosshatch")
            opts.bgpattern = (char*)CrossHatch;
        ...
        if (cinpf == RGB8)
            ...
            opts.cinPixel = (char*)RGBA8;
        else if (cinpf == RGB8_PLANAR)
            opts.cinPixel = (char*)RGB8_PLANAR;
        else if (cinpf == RGB10_A2)
            opts.cinPixel = (char*)RGB10_A2;
        else if (cinpf == A2_BGR10)
            opts.cinPixel = (char*)A2_BGR10;
        else if (cinpf == RGB16)
            opts.cinPixel = (char*)RGB16;
        else if (cinpf == RGBA16)
            opts.cinPixel = (char*)RGBA16;
        else if (cinpf == RGB16_PLANAR)
            opts.cinPixel = (char*)RGB16_PLANAR;
        ...
        if (dpxpf == RGB8)
            ...
            opts.dpxPixel = (char*)RGBA8;
        else if (dpxpf == RGB8_PLANAR)
            opts.dpxPixel = (char*)RGB8_PLANAR;
        else if (dpxpf == RGB10_A2)
            opts.dpxPixel = (char*)RGB10_A2;
        else if (dpxpf == A2_BGR10)
            opts.dpxPixel = (char*)A2_BGR10;
        else if (dpxpf == RGB16)
            opts.dpxPixel = (char*)RGB16;
        else if (dpxpf == RGBA16)
            opts.dpxPixel = (char*)RGBA16;
        else if (dpxpf == RGB16_PLANAR)
            opts.dpxPixel = (char*)RGB16_PLANAR;
    ...
    void RvPreferences::closeEvent(QCloseEvent* event)
    {
    ...
    void RvPreferences::write()
    {
        ...
        default:
            break;
        case 1:
            stereo = (char*)StereoAnaglyph;
            break;
        case 2:
            stereo = (char*)StereoPair;
            break;
        case 3:
            stereo = (char*)StereoMirror;
            break;
        case 4:
            stereo = (char*)StereoLeft;
            break;
        case 5:
            stereo = (char*)StereoRight;
            break;
        case 6:
            stereo = (char*)StereoChecker;
            break;
        case 7:
...
// backwards compatibility.
        ...
        case 2:
            ...
            break;
        case 3:
            method = (char*)Solid100;
            break;
        case 4:
            method = (char*)Checker;
            break;
        case 5:
            method = (char*)CrossHatch;
            break;
        }
    ...
    void RvPreferences::updateStyleSheet()
    {
        Options& opts = Options::sharedOptions();
    ...
    static void newCacheSize(float gigs, IPGraph::CachingMode m)
    {
    ...
    void RvPreferences::lookBehindFinished()
    {
    ...
    void RvPreferences::cacheOutsideRegionChanged(int state)
    {
    ...
    void RvPreferences::rthreadFinished() {}
    ...
    void RvPreferences::bitDepthChanged(int index)
    {
        switch (index)
        {
        case 0: // 32
    ...
    void RvPreferences::appleClientStorageChanged(int state)
    {
    ...
    void RvPreferences::useThreadedUploadChanged(int state)
    {
        const TwkApp::Application::Documents& docs = IPCore::App()->documents();
    ...
    void RvPreferences::videoSyncChanged(int state)
    {
        const TwkApp::Application::Documents& docs = IPCore::App()->documents();
    ...
    void RvPreferences::displayOutputChanged(int state)
    {
        const TwkApp::Application::Documents& docs = IPCore::App()->documents();
    ...
    static StreamingFrameBufferIO* findPlugin(const string& name)
    {
    ...
    void RvPreferences::exrAutoThreads(int state)
    {
        if (state == Qt::Checked)
        {
    ...
    void RvPreferences::exrThreadNumChanged(const QString& text)
    {
        if (text == "0")
        {
    ...
    void RvPreferences::cinPixelsChanged(int index)
    {
        if (StreamingFrameBufferIO* cin = findPlugin("IOcin"))
        {
            switch (index)
            {
            case 0:
                cin->setIntAttribute("format", IOcin::RGB8);
                break;
            case 1:
                cin->setIntAttribute("format", IOcin::RGBA8);
                break;
            case 2:
                cin->setIntAttribute("format", IOcin::RGB8_PLANAR);
                break;
            case 3:
                cin->setIntAttribute("format", IOcin::RGB10_A2);
                break;
            case 4:
                cin->setIntAttribute("format", IOcin::A2_BGR10);
                break;
            case 5:
                cin->setIntAttribute("format", IOcin::RGB16);
                break;
            case 6:
                cin->setIntAttribute("format", IOcin::RGBA16);
                break;
            case 7:
                cin->setIntAttribute("format", IOcin::RGB16_PLANAR);
                break;
            }
        }
    }
    ...
    void RvPreferences::dpxPixelsChanged(int index)
    {
        if (StreamingFrameBufferIO* dpx = findPlugin("IOdpx"))
        {
            switch (index)
            {
            case 0:
                dpx->setIntAttribute("format", IOdpx::RGB8);
                break;
            case 1:
                dpx->setIntAttribute("format", IOdpx::RGBA8);
                break;
            case 2:
                dpx->setIntAttribute("format", IOdpx::RGB8_PLANAR);
                break;
            case 3:
                dpx->setIntAttribute("format", IOdpx::RGB10_A2);
                break;
            case 4:
                dpx->setIntAttribute("format", IOdpx::A2_BGR10);
                break;
            case 5:
                dpx->setIntAttribute("format", IOdpx::RGB16);
                break;
            case 6:
                dpx->setIntAttribute("format", IOdpx::RGBA16);
                break;
            case 7:
                dpx->setIntAttribute("format", IOdpx::RGB16_PLANAR);
                break;
            }
        }
    }
    ...
    void RvPreferences::audioModuleChanged(int index)
    {
        ...
        try
        {
            renderer = AudioRenderer::renderer();
        }
        catch (...)
        { /* cerr << "renderer() threw" << endl; */
        }
        ...
        //
        Options& opts = Options::sharedOptions();
        if ((deviceName == "" || deviceName == "Default") && opts.audioDevice)
        ...
        for (int i = 0; i < layouts.size();)
        {
            if (layouts[i] == currentLayout)
                break;
            if (layouts[i] == TwkAudio::Stereo_2)
                hasStereoLayout = true;
            ++i;
            if (i == layouts.size())
            {
    ...
    void RvPreferences::audioDeviceChanged(int index)
    {
        if (AudioRenderer::audioDisabledAlways())
            return;
        ...
        // cerr << "audioDeviceChanged index=" << index << " hasChanged=" <<
        // (int) hasChanged << endl;
    ...
    void RvPreferences::audioChannelsChanged(int index)
    {
        if (AudioRenderer::audioDisabledAlways())
            return;
    ...
    void RvPreferences::audioFormatChanged(int index)
    {
        if (AudioRenderer::audioDisabledAlways())
            return;
    ...
    void RvPreferences::audioCachePacketChanged() {}
    ...
    void RvPreferences::clickedPackage(QModelIndex index)
    {
        if (index.column() == 0)
        {
        ...
        else if (index.column() == 1)
        {
            ...
            if (package.installed)
            {
                allowLoading(package, pitem->checkState() != Qt::Checked, 0);
    ...
    void RvPreferences::addPackageItem(const Package& p, size_t index)
    {
    ...
    void RvPreferences::loadPackages()
    {
        m_packageModel->clear();
        QStringList headers;
        headers << "Installed" << "Load" << "Name" << "File" << "Version"
                << "Author" << "Organization";
        ...
        for (int i = 0; i < headers.size(); i++)
            m_ui.extTreeView->resizeColumnToContents(i);
    }

    void RvPreferences::declarePackage(Package& p, size_t i)
    {
    ...
    void RvPreferences::packageSelection(const QItemSelection& selected, const QItemSelection& deselected)
    {
        ...
        {
            str << "<style type=\"text/css\">a:link {color: "
                   "rgb(100,100,200);}</style>";
        }
        ...
        if (!p.compatible)
        {
            str << "<h3>" << "-- NOT COMPATIBLE WITH THIS VERSION OF RV --"
                << "</h3>";
        }

        if (p.system)
        {
            str << "<h3>-- THIS IS A BASE SYSTEM PACKAGE REQUIRED BY RV "
                   "--</h3>";
        }
        ...
        str << "</table>"
            << "</center>"
            << "<h3>Description</h3>" << p.description.toUtf8().data();
    ...
    void RvPreferences::addPackage(bool)
    {
        //
        ...
        // Rationale:
        // When using native QFileDialog (default) on linux, the following
        // warning appears in the console:
        // <<GTk-Message: 17:38:26.978: GtkDialog mapped without a transient
        // parent.
        //   This is discouraged.>>
        // The warning does not appear when using non native dialog.
        // Moreover, some users have reported that the dialog fails to show
        // since RV was updated to Qt 5.15.3.
    ...
    void RvPreferences::removePackage(bool)
    {
        QItemSelectionModel* s = m_ui.extTreeView->selectionModel();
        QModelIndexList indices = s->selection().indexes();
    ...
    bool RvPreferences::fixLoadability(const QString& msg)
    {
        QMessageBox box(this);
    ...
    bool RvPreferences::fixUnloadability(const QString& msg)
    {
        QMessageBox box(this);
        box.setWindowTitle(tr("Loadable Package Dependencies"));
        box.setText(tr("Can't make package unloadable because some loaded "
                       "packages depend on it. Unload them too?\n\nDetails:\n")
                    + msg);
    ...
    bool RvPreferences::installDependantPackages(const QString& msg)
    {
        QMessageBox box(this);
    ...
    bool RvPreferences::overwriteExistingFiles(const QString& msg)
    {
        QMessageBox box(this);
        box.setWindowTitle(tr("Existing Package Files"));
        box.setText(tr("Package files conflict with existing files? Overwrite "
                       "Existing Files?\n\nDetails:\n")
                    + msg);
    ...
    void RvPreferences::errorMissingPackageDependancies(const QString& msg)
    {
        QMessageBox box(this);
        box.setWindowTitle(tr("Missing Package Dependencies"));
        box.setText(tr("Can't install package because some of its dependencies "
                       "are missing.\n\nDetails:\n")
                    + msg);
    ...
    bool RvPreferences::uninstallDependantPackages(const QString& msg)
    {
        QMessageBox box(this);
        box.setWindowTitle(tr("Some Packages Depend on This One"));
    ...
    void RvPreferences::informCannotRemoveSomeFiles(const QString& msg)
    {
        QMessageBox box(this);
    ...
    void RvPreferences::errorModeFileWriteFailed(const QString& file)
    {
        QMessageBox box(this);
    ...
    void RvPreferences::informPackageFailedToCopy(const QString& msg)
    {
        QMessageBox box(this);
    ...
    bool RvPreferences::uninstallForRemoval(const QString& msg)
    {
        QMessageBox box(this);
        box.setWindowTitle(tr("Package is currently installed"));
        box.setText(tr("In order to remove the package it must be "
                       "uninstalled.\n\nDetails:\n")
                    + msg);
    ...
    void RvPreferences::updateVideo()
    {
        ...
        for (size_t i = 0; i < vmods.size(); i++)
        {
            ...
            if (i == m_currentVideoModule)
            {
                ...
                if (info != "")
                    sdk = QString("%1 -- %2").arg(sdk).arg(info);
    ...
    void RvPreferences::updateVideoProfiles(TwkApp::VideoDevice* device)
    {
        //
        //  Profiles
        //
        ...
        for (size_t i = 0; i < combos.size(); i++)
        {
            ...
            {
            case 0:
                combos[i]->addItem("- Use Default Profile -");
                break;
            case 1:
                combos[i]->addItem("- Use Module Profile -");
                break;
            case 2:
                combos[i]->addItem("- Use Device Profile -");
                break;
            }
        }
    ...
    void RvPreferences::updateVideo4KTransport(TwkApp::VideoDevice* d)
    {
    ...
    void RvPreferences::updateVideoAudioFormat(TwkApp::VideoDevice* d)
    {
    ...
    void RvPreferences::updateVideoDataFormat(TwkApp::VideoDevice* d)
    {
    ...
    void RvPreferences::updateVideoSync(TwkApp::VideoDevice* d)
    {
    ...
    void RvPreferences::updateVideoSyncSource(TwkApp::VideoDevice* d)
    {
    ...
    void RvPreferences::videoModuleChanged(int index)
    {
    ...
    void RvPreferences::videoDeviceChanged(int index)
    {
        m_currentVideoDevice = index;
        updateVideo();
    }

    void RvPreferences::videoFormatChanged(int v)
    {
        if (VideoDevice* d = currentVideoDevice())
        {
            ...
            // The user has set a new video format which overrides the video
            // format specified on the command line if any
    ...
    void RvPreferences::video4KTransportChanged(int v)
    {
        if (VideoDevice* d = currentVideoDevice())
        {
    ...
    void RvPreferences::videoAudioFormatChanged(int v)
    {
        if (VideoDevice* d = currentVideoDevice())
        {
            d->setAudioFormat(v);
    ...
    void RvPreferences::videoDataFormatChanged(int v)
    {
        if (VideoDevice* d = currentVideoDevice())
        {
            d->setDataFormat(v);
            ...
            // The user has set a new video data format which overrides the
            // video data format specified on the command line if any
    ...
    void RvPreferences::syncMethodChanged(int v)
    {
        if (VideoDevice* d = currentVideoDevice())
        {
            d->setSyncMode(v);
    ...
    void RvPreferences::syncSourceChanged(int v)
    {
        if (VideoDevice* d = currentVideoDevice())
        {
            d->setSyncSource(v);
    ...
    void RvPreferences::videoAudioCheckBoxChanged(int v)
    {
        if (VideoDevice* d = currentVideoDevice())
        {
            const VideoModule* m = d->module();
            ...
            // This user action overrides the presentAudio command line option
            // if any
            Rv::Options::sharedOptions().presentAudio = -1;
        }
    }

    void RvPreferences::videoSwapStereoEyesChanged(int v)
    {
        if (VideoDevice* d = currentVideoDevice())
        {
            const VideoModule* m = d->module();
    ...
    void RvPreferences::videoUseLatencyCheckBoxChanged(int v)
    {
        if (VideoDevice* d = currentVideoDevice())
        {
            const VideoModule* m = d->module();
    ...
    void RvPreferences::videoAdditionalOptionsChanged()
    {
        if (VideoDevice* d = currentVideoDevice())
        {
            const VideoModule* m = d->module();
    ...
    void RvPreferences::presentationCheckBoxChanged(int state)
    {
        if (m_lockPresentCheck)
            return;
    ...
    TwkApp::VideoDevice* RvPreferences::currentVideoDevice() const
    {
    ...
    void RvPreferences::audioOffsetFinished()
    {
        const TwkApp::Application::Documents& docs = IPCore::App()->documents();
    ...
    void RvPreferences::latencyChanged(const QString& s, bool fixed)
    {
        double v = s.toDouble();
    ...
    void RvPreferences::modProfileChanged(int index)
    {
        TwkApp::VideoDevice* d = currentVideoDevice();
    ...
    void RvPreferences::devProfileChanged(int index)
    {
        TwkApp::VideoDevice* d = currentVideoDevice();
    ...
    void RvPreferences::formatProfileChanged(int index)
    {
        TwkApp::VideoDevice* d = currentVideoDevice();
